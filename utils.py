...
def prepare_trend_data(df, Rubro, dict_palabras, use_external, ciclo, qtr_test = 2, delay = 0):
    if use_external == True:
        ...
        if (use_external==False and ciclo==1):
            data_trends_transposed = pd.DataFrame()
            data_trends_train = pd.DataFrame()
            data_trends_test = pd.DataFrame()

        else:
            if (use_external==False and ciclo==2):
                data_trends_transposed = pd.read_csv('./data/PIB_adj_ciclo_1.csv',index_col=0)
                data_trends_train = data_trends_transposed[:-(qtr_test)]
                data_trends_test = data_trends_transposed.tail(qtr_test)

    return data_trends_transposed,data_trends_train,data_trends_test

def predict_sarimax(df, Rubro, external_train, external_test, tipo, use_external, ciclo, trend):
    #Cantidad de trimestres a proyectar
    n_periods = 2

    #Si usamos variables externas (datos de google trends), usamos esos datos, independientemente del ciclo
    if use_external==True:
        model = pm.auto_arima(df[df.Rubro==Rubro].value,
                              exogenous=external_train,
                              test='adf', seasonal_test = 'ch',
                              max_p=6, max_q=6, max_d=3, max_P=6, max_Q=6, max_D=3,
                              m=4, alpha = 0.05, trend = trend, seasonal=True,
                              d=None, D=None, trace=True, error_action='ignore', suppress_warnings=True,
                              random_state=123, random=True, maxiter = 400, n_fits=400, stepwise=False)
        ...
        # Si no usamos variables externas (datos de google trends) y estamos en el ciclo 1, no usamos ningún dato externo en el modelo
        if (use_external==False and ciclo==1):
            model = pm.auto_arima(df[df.Rubro==Rubro].value,
                                  # exogenous=external_train,
                                  test='adf', seasonal_test = 'ch',
            ...
            # Si no usamos variables externas (datos de google trends) y estamos en el ciclo 2, usamos el dato externo del PIB generado en el ciclo 1
            if (use_external==False and ciclo==2):
                model = pm.auto_arima(df[df.Rubro==Rubro].value,
                                      exogenous=external_train,
                                      test='adf', seasonal_test = 'ch',
...
def predict_rubro(df, Rubro, dict_palabras, use_external=True, delay=0, ciclo=1, trend='ct', extract=True):
    ...
    data_prepared, data_train, data_test = prepare_trend_data(data_trends, Rubro, dict_palabras, use_external=use_external, ciclo=ciclo, qtr_test = 2, delay=delay)

    #Predecir con ajustes
    print("Prediciendo con ajustes...")
    predictions_ajustada = predict_sarimax(df, Rubro, data_train, data_test,'adj', use_external=use_external, ciclo=ciclo, trend=trend)

    #Predecir sin ajustes
    print("Prediciendo sin ajustes")
    predictions_original = predict_sarimax(df, Rubro, None, None, 'orig', use_external=use_external, ciclo=ciclo, trend=trend)
